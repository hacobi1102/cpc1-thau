...
if 'list_aliases' not in st.session_state: st.session_state.list_aliases = [] # Lưu trữ toàn bộ list alias
if 'new_aliases' not in st.session_state: st.session_state.new_aliases = [] # CHỈ lưu trữ những alias vừa mới tạo trong phiên này
...
def process_single_row(input_row, cpc1_db, wratio_threshold=80):
    for field in REQUIRED_COLUMNS:
        if pd.isna(input_row.get(field)) or str(input_row.get(field)).strip() == "":
            return {
                "Dữ liệu đầu vào": "Lỗi Dữ Liệu",
                "Kết quả đối chiếu danh mục CPC1": f"Vui lòng bổ sung cột [{field}]",
                "Tên mặt hàng (CPC1)": "",
                "Hãng (CPC1)": "",
                "Tỉ lệ khớp": "N/A",
    ...
    for _, cpc1_row in cpc1_db.iterrows():
        ...
        if hc_score == 100 and dosage_match:
            ...
            current_note = "Khớp chính xác hoàn toàn."
            weight = 3
    ...
    if used_alias and best_match is not None:
        note += f" (Đã quy đổi qua Alias: {input_hc} - {input_dosage_raw})"
    ...
    return {
        "Dữ liệu đầu vào": input_display,
        "Kết quả đối chiếu danh mục CPC1": cpc1_display,
...
with col1:
    ...
    # Dùng uploader_key để thay đổi định danh khung tải file mỗi khi reset
    master_file = st.file_uploader("Tải tệp Excel gốc tại đây", type=["xlsx", "xls"], key=f"master_{st.session_state.uploader_key}")
    master_msg = st.empty() # Tạo sẵn khung giữ chỗ cho thông báo của cột 1
...
with col2:
    ...
    target_msg = st.empty() # Tạo sẵn khung giữ chỗ cho thông báo của cột 2

# Logic: Đọc file gốc và khởi tạo các cột Alias nếu chưa có. 
# CẢI TIẾN: Dùng tên file + dung lượng file để ép reload khi có thay đổi.
if master_file:
    ...
    if file_id != st.session_state.master_file_name:
        ...
        if missing_cpc1:
            st.session_state.master_status = ("error", f"❌ File gốc CPC1 thiếu các cột chuẩn: {missing_cpc1}. Vui lòng kiểm tra lại cấu trúc.")
        else:
            # Reset toàn bộ state liên quan đến alias cũ
            st.session_state.alias_hc = {}
            st.session_state.alias_hl = {}
            st.session_state.list_aliases = []
            
            # Quét xem file gốc có sheet Alias không để nạp vào trí nhớ
            ...
            if has_alias and (chc > 0 or chl > 0):
                st.session_state.master_status = ("success", f"✅ Đã nạp CPC1 và tìm thấy sheet Alias: {chc} Hoạt chất, {chl} Hàm lượng.")
            else:
                st.session_state.master_status = ("success", "✅ Đã nạp danh mục gốc CPC1.")
                
    # Hiển thị thông báo ra khung master_msg (Lưu trong session_state để không bị mất khi thao tác Slider)
...
if target_file:
    target_msg.success(f"✅ Đã nạp danh mục cần đối chiếu: **{target_file.name}**")
...
if master_file and target_file:
    st.markdown("---")
    # Thay đổi thứ tự và tỉ lệ cột: Slider (2) -> Start (1) -> Reset (1)
    ...
    with col_btn_start:
        # Chèn khoảng đệm tàng hình để đẩy nút xuống ngang hàng với Slider
        st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
        start_btn = st.button("🚀 BẮT ĐẦU ĐỐI CHIẾU", use_container_width=True, type="primary")
        
    with col_btn_reset:
        # Thêm CSS và khoảng đệm cho nút Reset
        st.markdown("""
        <style>
    ...
    # Xử lý logic khi bấm nút LÀM MỚI
    if reset_btn:
        # THÊM 'new_aliases' VÀ 'master_status' VÀO DANH SÁCH CẦN XÓA
        keys_to_clear = ['alias_hc', 'alias_hl', 'list_aliases', 'new_aliases', 'master_file_name', 'processed', 'df_results', 'df_target', 'df_cpc1', 'master_status']
    ...
    if start_btn:
        try:
            ...
            if missing_target: st.error(f"❌ File cần đối chiếu thiếu các cột: {missing_target}"); st.stop()
            ...
            for index, row in df_target.iterrows():
                ...
                # Gắn thêm Số Thứ Tự (STT) gốc của file để không bị mất dấu khi sort
...
if st.session_state.processed:
    ...
    brand_counts = high_match_df['Hãng (CPC1)'].value_counts()
    ...
    st.dataframe(df_display.style.apply(highlight_similarity, axis=1), width='stretch', height=400, hide_index=True)
    ...
    failed_mask = df_results['Tỉ lệ khớp'].isin(['Dưới 50%', '50% - 79%'])
    failed_indices = df_results[failed_mask].index.tolist()

    if not failed_indices:
        st.success("🎉 Tuyệt vời! Toàn bộ dữ liệu đều khớp rất tốt, không có dòng nào cần ghép nối thủ công.")
    else:
        def format_input(idx):
            res_row = df_results.iloc[idx]
            return f"Dòng {idx+1}: {res_row['Dữ liệu đầu vào']} ➔ {res_row['Tỉ lệ khớp']}"
